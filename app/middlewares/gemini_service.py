...
def chat_with_manager(manager_message, first_message):
    """
    This function acts as a simple chatbot for the manager.
    It reads a base chat prompt from a file (which can include guidelines for the conversation),
    appends the manager's message, and then uses the Gemini API to generate a reply.
    ...
    """
    # Set the model (using Gemini API) and generate a response.
    MODEL = "gemini-1.5-flash"
    model = genai.GenerativeModel(MODEL)
    # Path to a file containing instructions for the chatbot conversation.
    ...
    if first_message:
        ...
        input_text = "Manager: " + manager_message
